...
struct EmptyStateStyle {
    func apply<V: View>(to view: V) -> some View {
        view
            .frame(maxWidth: ImageBrowserViewStyles.Layout.maxWidthInfinity,
                   maxHeight: ImageBrowserViewStyles.Layout.maxHeightInfinity)
    }
}
...
struct PlayPauseButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ZStack {
                Circle()
                    .fill(Color.black.opacity(ImageBrowserViewStyles.Buttons.playPauseButtonBackgroundOpacity))
                    .frame(width: ImageBrowserViewStyles.Buttons.playPauseButtonSize, 
                           height: ImageBrowserViewStyles.Buttons.playPauseButtonSize)
                
                Image(systemName: isAutoPlaying ? "pause.fill" : "play.fill")
                    .font(.system(size: ImageBrowserViewStyles.Buttons.playPauseIconSize))
                    .foregroundColor(.white)
            }
        }
        .buttonStyle(PlayPauseButtonStyle(isPressed: $isPressed))
        .cornerRadius(ImageBrowserViewStyles.Buttons.playPauseButtonCornerRadius)
...
struct ProgressBarStyles {
    /// 进度条高度（像素）
    static let progressBarHeight: CGFloat = 1
    
    /// 进度条背景透明度
    static let progressBarBackgroundOpacity: Double = 0.3
    
    /// 进度条前景透明度
    static let progressBarForegroundOpacity: Double = 1
        ...
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                ...
                if isAutoPlaying {
                    Rectangle()
                        ...
                        .frame(
                            ...
                            width: animationProgress * geometry.size.width * CGFloat(currentIndex) / CGFloat(totalItems),//当前进度宽度
                ...
                Rectangle()
                    ...
                    .frame(
                        width: geometry.size.width * CGFloat(currentIndex + 1) / CGFloat(totalItems),//+1 在播放第一张时就有进度条
                        height: ProgressBarStyles.progressBarHeight
                    )
                    .cornerRadius(2)
...
struct LayoutToggleButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ZStack {
                Circle()
                    .fill(Color.black.opacity(ImageBrowserViewStyles.Buttons.layoutToggleButtonBackgroundOpacity))
                    .frame(width: ImageBrowserViewStyles.Buttons.layoutToggleButtonSize, 
                           height: ImageBrowserViewStyles.Buttons.layoutToggleButtonSize)
                
                Image(systemName: isSmartLayout ? "square.grid.3x3" : "rectangle.grid.1x2")
                    .font(.system(size: ImageBrowserViewStyles.Buttons.layoutToggleIconSize))
                    .foregroundColor(.white)
            }
        }
